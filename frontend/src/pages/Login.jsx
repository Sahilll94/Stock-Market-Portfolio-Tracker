...
import { ArrowLeft, TrendingUp, BarChart3, Shield, Zap } from 'lucide-react';
...
export default function Login() {
  ...
  const features = [
    { icon: TrendingUp, title: 'Real-Time Tracking', desc: 'Monitor stocks instantly' },
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center p-4 transition-colors duration-300 ${
      ...
      <div className={`absolute top-0 left-0 w-96 h-96 rounded-full blur-3xl opacity-20 -translate-x-1/2 -translate-y-1/2 animate-pulse transition-all duration-1000 ${isDark ? 'bg-blue-600' : 'bg-blue-400'}`}></div>
      <div className={`absolute bottom-0 right-0 w-96 h-96 rounded-full blur-3xl opacity-20 translate-x-1/2 translate-y-1/2 animate-pulse transition-all duration-1000 delay-700 ${isDark ? 'bg-cyan-600' : 'bg-cyan-400'}`}></div>
      ...
      <div className="w-full max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
        ...
        <div className="hidden lg:block space-y-6 animate-fadeInLeft">
          <div className="space-y-2">
            ...
            <p className={`text-lg transition-colors duration-300 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              Smart tracking for smarter investing
            </p>
          </div>
            ...
            {features.map((feature, idx) => {
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-lg border transition-all duration-300 cursor-pointer group transform hover:scale-105 hover:-translate-y-1 ${
                    hoveredFeature === idx
                ...
                >
                  <div className={`p-2.5 rounded-lg w-fit mb-3 transition-all duration-300 ${
                    hoveredFeature === idx
          ...
          <div className={`p-4 rounded-lg border transition-all duration-300 ${
            isDark ? 'bg-green-600/10 border-green-500/30' : 'bg-green-50/50 border-green-300/30'
          }`}>
            <p className={`text-xs font-semibold mb-2 transition-colors duration-300 ${isDark ? 'text-green-400' : 'text-green-700'}`}>
              ✓ Production Ready
            </p>
            <p className={`text-xs transition-colors duration-300 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              Real-time data from 40k+ global stocks. Secure authentication with Google OAuth & email. Advanced analytics & portfolio tracking.
            </p>
          </div>
        </div>

        {/* Right: Login Form */}
        <div className={`rounded-xl border p-8 transition-all duration-500 transform animate-fadeInUp ${
          isDark
      ...
      <div className={`absolute top-20 right-10 w-2 h-2 rounded-full animate-float opacity-30 ${isDark ? 'bg-blue-500' : 'bg-blue-400'}`}></div>
      <div className={`absolute bottom-32 left-10 w-3 h-3 rounded-full animate-float opacity-20 ${isDark ? 'bg-cyan-500' : 'bg-cyan-400'}`} style={{ animationDelay: '1s' }}></div>
    </div>
  );
}
